...
class IXPMonitorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("IXP Latency and Traffic Monitor")
    ...
    def create_widgets(self):
        # Input field to enter the IP address of the IXP server
        ttk.Label(self.root, text="Enter IXP Server IP Address:").grid(row=0, column=0, padx=10, pady=5)
        self.ip_entry = ttk.Entry(self.root, textvariable=self.ip_address, width=50)
    ...
    def monitor_network(self):
        # Start monitoring network traffic and pinging IXP IP
        self.latency_data = []
        self.network_data = []
        start_time = time.time()
        
        while self.is_monitoring:
            # Get the IXP IP address from the entry
